...
def test_wrong_latitude(django_client):
    ...
    assert response.data == 'Latitude and longitude must be numbers.'
...
def test_negative_latitude_longitude(django_client):
    ...
    with asynctest.mock.patch('weather.providers.Noaa.get_current_temp', side_effect=[both_temperature_response]):
        response = django_client.get('/weather/?latitude={}&longitude={}&filters=noaa'.format(latitude,
                                                                                                          longitude))
...
def test_no_filters(django_client):
    ...
    assert response.status_code == 400
    assert response.data == 'At least one filter must be provided to validate into a provider.'
...
def test_no_latitude(django_client):
    ...
    assert response.status_code == 400
    assert response.data == 'Latitude and longitude params must be provided.'
...
def test_wrong_filters(django_client):
    ...
    assert response.status_code == 400
    assert response.data == 'Some providers given are not valid.'
...
def test_any_provider_connection_is_down(django_client):
    ...
    with asynctest.mock.patch('weather.providers.Noaa.get_current_temp',
        ...
        assert response.data == 'At this moment we are not being able to reach al the providers.'
